...
void RRTSharp::addNode(const std::shared_ptr<Node>& node) noexcept
{
    ...
    const int indexX = static_cast<int>(std::floor((node->state().point().y() - m_minX) / m_resolution));
    const int indexY = static_cast<int>(std::floor((node->state().point().x() - m_minY) / m_resolution));
...
void RRTSharp::initObjects() noexcept
{
    // Object 1
    double minX = -80.0;
    double maxX = -30.0;
...
void RRTSharp::initVariables() noexcept
{
...
State RRTSharp::sampleState() const noexcept
{
    ...
    static std::uniform_int_distribution<std::mt19937::result_type> uniformDistributionPoseX(0, m_numX * scale);
    static std::uniform_int_distribution<std::mt19937::result_type> uniformDistributionPoseY(0, m_numY * scale);
