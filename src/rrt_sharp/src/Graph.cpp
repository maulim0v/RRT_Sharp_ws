...
#include "rrt_mpc/Graph.hpp"
#include "rrt_mpc/Collision.hpp"
...
void graph_t::getBucketsAsList(const Config &c, std::vector<int> &result) const {
  int b_x, b_y, b_th;
  int d_x, d_y, d_th;
  getBuckets(c, &b_x, &b_y, &b_th, &d_x, &d_y, &d_th);
